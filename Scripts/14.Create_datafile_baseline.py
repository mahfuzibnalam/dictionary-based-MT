...
for dataset in ["stanza.5K", "stanza.10K", "stanza.50K", "stanza.100K", "stanza.200K", "stanza.all"]:
    if os.path.exists(f"data/{pair}/{dataset}"):
        ...
        for size in beautysize:
            os.system(f"mkdir data/{pair}/{src}-{tgt}.{dataset}.bas.{size}")
            os.system(f"mkdir data/{pair}/{tgt}-{src}.{dataset}.bas.{size}")
            for typ in ["train", "test", "valid"]:
                if typ == "train":
                    with open(f"data/{pair}/{dataset}/train({size})_bas.{src}", "r") as rs:
                        with open(f"data/{pair}/{dataset}/train({size})_bas.{tgt}", "r") as rt:
                            srclines = rs.readlines()
                            tgtlines = rt.readlines()
                            
                            with open(f"data/{pair}/{src}-{tgt}.{dataset}.bas.{size}/train.{src}", "w") as wsts:
                                with open(f"data/{pair}/{src}-{tgt}.{dataset}.bas.{size}/train.{tgt}", "w") as wstt:
                                    with open(f"data/{pair}/{tgt}-{src}.{dataset}.bas.{size}/train.{src}", "w") as wtss:
                                        with open(f"data/{pair}/{tgt}-{src}.{dataset}.bas.{size}/train.{tgt}", "w") as wtst:
                                            for index, _ in enumerate(tgtlines):
                                                srcline = srclines[index]
                                                tgtline = tgtlines[index]
                                                wsts.write(srcline)
                    ...
                    with open(f"data/{pair}/{dataset}/tagged_{typ}.{src}", "r") as rs:
                        with open(f"data/{pair}/{dataset}/tagged_{typ}.{tgt}", "r") as rt:
                            ...
                            with open(f"data/{pair}/{src}-{tgt}.{dataset}.bas.{size}/{typ}.{src}", "w") as wsts:
                                with open(f"data/{pair}/{src}-{tgt}.{dataset}.bas.{size}/{typ}.{tgt}", "w") as wstt:
                                    with open(f"data/{pair}/{tgt}-{src}.{dataset}.bas.{size}/{typ}.{src}", "w") as wtss:
                                        with open(f"data/{pair}/{tgt}-{src}.{dataset}.bas.{size}/{typ}.{tgt}", "w") as wtst:
                                            for index, _ in enumerate(tgtlines):
                                                srcline = srclines[index]
                                                tgtline = tgtlines[index]
                                                wsts.write(srcline)
