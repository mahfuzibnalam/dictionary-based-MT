...
parser.add_argument("--s", help="echo the string you use here", default="ga")
...
if os.path.exists(f"data/{pair}/{dataset}"):
    ...
    with open(f"new_train({beautysize[index]})_dedup_bas.{src}", "r") as rs:
        with open(f"new_train({beautysize[index]})_dedup_bas.{tgt}", "r") as rt:
            srclines = rs.readlines()
            tgtlines = rt.readlines()
            scores = topindexes(srclines, tgtlines, dataset)
            with open(f"sorted({beautysize[index]})_bas.{src}", "w") as ws:
                with open(f"sorted({beautysize[index]})_bas.{tgt}", "w") as wt:
                        for id in scores.keys():
                            ws.write(srclines[id])
                            wt.write(tgtlines[id])
